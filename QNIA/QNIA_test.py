...
from QNIA_concat import ERROR, readExcelFile, SELECT_DF_KEY
...
def QNIA_identity(data_path, df_key, DF_KEY, checkNotFound=False, checkDESC=True, checkOnly='', checkIgnore=[], tStart=time.time(), start_year=1901):
    
    for handler in logging.root.handlers[:]:
        logging.root.removeHandler(handler)
    FORMAT = '%(asctime)s %(message)s'
    logging.basicConfig(level=logging.INFO, format=FORMAT, handlers=[logging.FileHandler("TEST.log", 'w', CCT.ENCODING)], datefmt='%Y-%m-%d %I:%M:%S %p')
            ...
            for check in CHECK:
                if str(df_key.loc[ind, check]).strip().lower() != str(DF_KEY.loc[ind, check]).strip().lower():
                    if check == 'start' and (str(DF_KEY.loc[ind, check]).strip() == 'Nan' or str(df_key.loc[ind, check]).strip() < str(DF_KEY.loc[ind, check]).strip()):
                        continue
                    elif (str(DF_KEY.loc[ind, check]).strip() == 'nan' or str(DF_KEY.loc[ind, check]).strip() == 'None') and str(df_key.loc[ind, check]).strip() == '':
                        continue
...
if local == True:
    ...
    DF_KEY = DF_KEY.set_index('name') 
    unknown_list, toolong_list, update_list, unfound_list = QNIA_identity(data_path, df_key, DF_KEY, checkDESC=checkDESC, start_year=styr)
